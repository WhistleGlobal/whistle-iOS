...
// MARK: - VideoExporterBottomSheetView

struct VideoExporterBottomSheetView: View {
  @Binding var isPresented: Bool
  @StateObject private var viewModel: ExporterViewModel

  init(isPresented: Binding<Bool>, video: EditableVideo) {
    _isPresented = isPresented
    _viewModel = StateObject(wrappedValue: ExporterViewModel(video: video))
  }
...
// MARK: - VideoQualityPopapView2_Previews

struct VideoQualityPopapView2_Previews: PreviewProvider {
  static var previews: some View {
    ZStack(alignment: .bottom) {
      Color.secondary.opacity(0.5)
      VideoExporterBottomSheetView(isPresented: .constant(true), video: EditableVideo.mock)
    }
  }
}
