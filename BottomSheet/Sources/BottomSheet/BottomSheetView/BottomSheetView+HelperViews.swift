...
extension BottomSheetView {
  func fullScreenBackground(with geometry: GeometryProxy) -> some View {
    VisualEffectView(visualEffect: configuration.backgroundBlurMaterial)
      .opacity(
      ...
      .onTapGesture(perform: tapToDismissAction)
  ...
  func bottomSheet(with geometry: GeometryProxy) -> some View {
    ...
    .frame(
      width: width(with: geometry),
      height: bottomSheetPosition.isDynamic && translation == 0 ? nil : height(with: geometry),
      alignment: isIPadFloatingOrMac ? .bottom : .top)
    // Clip BottomSheet for transition to work correctly for iPad and Mac
    .clipped()
    // BottomSheet background
    .background(
      bottomSheetBackground(with: geometry))
    // On iPad floating and Mac the BottomSheet has a padding
    .padding(
      isIPadFloatingOrMac ? 10 : 0)
    ...
    .transition(.move(
      edge: isIPadFloatingOrMac ? .top : .bottom))
  }
  ...
  func bottomSheetContent(with geometry: GeometryProxy) -> some View {
    ...
    .onReceive(Just(bottomSheetPosition.isBottom)) { isBottom in
      if isBottom {
    ...
    .onReceive(Just(configuration.isCloseButtonShown)) { isCloseButtonShown in
    ...
    .onReceive(Just(headerContent)) { headerContent in
  ...
  func appleScrollView(with geometry: GeometryProxy) -> some View {
    ...
    .gesture(
      isScrollEnabled ? nil : appleScrollViewDragGesture(with: geometry))
  }
  #endif
  ...
  func header(with geometry: GeometryProxy) -> some View {
    ...
    .background(headerGeometryReader)
    // Make the header drag-able
    .gesture(
      configuration.isResizable ? dragGesture(with: geometry) : nil)
  }
    ...
    Group {
      ...
      if let backgroundView = self.configuration.backgroundView {
        ...
        VisualEffectView(visualEffect: .system)
          ...
          .cornerRadius(
            10,
            corners: self.isIPadFloatingOrMac
              ? .allCorners
              : [
                .topRight,
                .topLeft,
  ...
  var mainContentBottomPadding: CGFloat {
    if !isIPadFloatingOrMac, configuration.accountForKeyboardHeight {
      #if !os(macOS)
