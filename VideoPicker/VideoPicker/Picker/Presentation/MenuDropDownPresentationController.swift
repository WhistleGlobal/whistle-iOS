...
// MARK: - MenuDropDownPresentationController
...
final class MenuDropDownPresentationController: UIPresentationController {
  ...
  override func presentationTransitionWillBegin() {
    ...
    if cornerRadius > 0 {
      let maskPath = UIBezierPath(
        roundedRect: presentationRoundedCornerView.bounds,
    ...
    if let presentedViewControllerView {
  ...
  override var frameOfPresentedViewInContainerView: CGRect {
    ...
    let presentedViewControllerFrame = CGRect(
      x: (containerViewBounds.width - presentedViewContentSize.width) / 2,
...
extension MenuDropDownPresentationController {
  @objc
  private func dimmingViewTapped(_: UITapGestureRecognizer) {
...
extension MenuDropDownPresentationController: UIViewControllerAnimatedTransitioning {
  ...
  func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
    ...
    if isPresenting {
      ...
      toViewInitialFrame.origin = CGPoint(
        x: toViewFinalFrame.origin.x,
        y: topOffsetHeight + navigationBarHeight + extraTopMenuHeight)
      ...
      fromViewFinalFrame.origin = CGPoint(
