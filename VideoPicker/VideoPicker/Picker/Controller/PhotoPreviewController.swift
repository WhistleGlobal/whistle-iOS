...
extension PhotoPreviewControllerDelegate {
  ...
  func previewController(_: PhotoPreviewController, useOriginalImage _: Bool) { }
  func previewControllerDidClickBack(_: PhotoPreviewController) { }
...
final class PhotoPreviewController: AnyImageViewController, PickerOptionsConfigurable {
  ...
  override func setStatusBar(hidden: Bool) {
    if
      let controller = (presentingViewController as? AnyImageNavigationController)?
        .topViewController as? AssetPickerViewController
    {
...
extension PhotoPreviewController {
  private func addNotifications() {
    ...
    NotificationCenter.default.addObserver(
      self,
      selector: #selector(previewCellDidDownloadResource(_:)),
      name: .previewCellDidDownloadResource,
      object: nil)
    #endif
  }
  ...
  private func setBar(hidden: Bool, animated: Bool = true, isNormal: Bool = true) {
    ...
    if isNormal {
      ...
      let color = UIColor.create(
        style: manager.options.theme.style,
        light: .white,
        dark: .black)
...
extension PhotoPreviewController {
  @objc
  private func containerSizeDidChange(_: Notification) {
  ...
  @objc
  func selectButtonTapped(_: NumberCircleButton) {
    guard let data = dataSource?.previewController(self, assetOfIndex: currentIndex) else { return }
...
extension PhotoPreviewController: PreviewCellDelegate {
  func previewCellDidBeginPan(_: PreviewCell) {
    delegate?.previewControllerWillDisappear(self)
    toolBarHiddenStateBeforePan = navigationBar.alpha == 0
  }

  func previewCell(_: PreviewCell, didPanScale scale: CGFloat) {
  ...
  func previewCell(_: PreviewCell, didEndPanWithExit isExit: Bool) {
    if isExit {
      dismiss(animated: true, completion: nil)
      setStatusBar(hidden: false)
  ...
  func previewCellDidSingleTap(_: PreviewCell) {
...
extension PhotoPreviewController: PickerPreviewIndexViewDelegate {
  func pickerPreviewIndexView(_: PickerPreviewIndexView, didSelect asset: Asset) {
    switch sourceType {
    case .album:
