...
extension PhotoPreviewController {
  /// ToolBar - Edit
  @objc
  func editButtonTapped(_: UIButton) {
        ...
        manager.requestPhoto(for: data.asset.phAsset, options: options) { [weak self] result in
          ...
          case .success(let response):
            if !response.isDegraded {
              view.hud.hide()
              showEditor(response.image, identifier: data.asset.identifier)
            }
  ...
  @objc
  func previewCellDidDownloadResource(_ notification: Notification) {
...
extension PhotoPreviewController {
  func autoSetEditorButtonHidden() {
...
extension PhotoPreviewController: ImageEditorControllerDelegate {
  ...
  func imageEditor(_ editor: ImageEditorController, didFinishEditing result: EditorResult) {
    ...
    guard let data = dataSource?.previewController(self, assetOfIndex: currentIndex) else { return }
    guard let cell = collectionView.cellForItem(at: IndexPath(item: currentIndex, section: 0)) as? PhotoPreviewCell
    else { return }
