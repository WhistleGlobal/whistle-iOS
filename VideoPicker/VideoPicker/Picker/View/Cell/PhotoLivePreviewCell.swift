...
extension PhotoLivePreviewCell {
  func requestLivePhoto() {
    ...
    DispatchQueue.global().async { [weak self] in
      guard let self else { return }
      let options = PhotoLiveFetchOptions(targetSize: PHImageManagerMaximumSize) { progress, _, _, _ in
      ...
      manager.requestPhotoLive(for: asset.phAsset, options: options) { result in
        switch result {
        case .success(let response):
          DispatchQueue.main.async { [weak self] in
            guard let self, asset.identifier == id else { return }
            livePhotoView.livePhoto = response.livePhoto
...
extension PhotoLivePreviewCell {
  /// 长按播放 live photo
  @objc
  private func onLongPress(_ gr: UILongPressGestureRecognizer) {
