...
final class VideoIOComponent: DeviceIOComponent {
  ...
  init(session: AVCaptureSession, options: CaptureOptionsInfo) {
    ...
    position = options.preferredPositions.first ?? .back
...
extension VideoIOComponent {
  private func addNotifications() {
    NotificationCenter.default.addObserver(
      self,
      selector: #selector(deviceSubjectAreaDidChange(_:)),
...
extension VideoIOComponent {
  @objc
  private func deviceSubjectAreaDidChange(_: Notification) {
...
extension VideoIOComponent {
  ...
  var minExposureTargetBias: Float {
    device?.minExposureTargetBias ?? 0
  }

  var exposureTargetBias: Float {
    device?.exposureTargetBias ?? 0
  }
...
extension VideoIOComponent: AVCapturePhotoCaptureDelegate {
  func photoOutput(_: AVCapturePhotoOutput, didCapturePhotoFor _: AVCaptureResolvedPhotoSettings) {
  ...
  private func export(photoData: Data) {
    ...
    guard let orientedImage = CIImage(data: photoData)?.oriented(forExifOrientation: cgOrientation) else { return }
...
// MARK: VideoIOComponent.ObservableProperty
