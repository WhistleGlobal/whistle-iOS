...
extension AVCaptureDevice {
  ...
  private func preferredFormats(
    ...
    colorSpace: AVCaptureColorSpace) -> [AVCaptureDevice.Format]
  {
    formats.filter { format in
      ...
      if dimensions.width == preset.width, dimensions.height == preset.height {
        let ranges = format.videoSupportedFrameRateRanges.filter {
          $0.maxFrameRate >= Double(preset.frameRate)
        }
        return !ranges.isEmpty
      }
      return false
    }.filter { format in
      (format.autoFocusSystem == autoFocusSystem) || (format.autoFocusSystem == .none) // For front camera
    }.filter { format in
