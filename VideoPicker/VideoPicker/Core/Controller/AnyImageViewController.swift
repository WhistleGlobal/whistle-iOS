...
extension AnyImageViewController {
  func check(
    ...
    limited: @escaping () -> Void,
    denied: @escaping (Permission) -> Void)
  {
  ...
  func check(permissions: [Permission], authorized: @escaping () -> Void, denied: @escaping (Permission) -> Void) {
    if !permissions.isEmpty {
      ...
      check(permission: permission, authorized: { [weak self] in
        ...
        check(permissions: _permissions, authorized: authorized, denied: denied)
...
extension AnyImageViewController {
  func check(
    permission: Permission,
    stringConfig: ThemeStringConfigurable,
    authorized: @escaping () -> Void,
    canceled: @escaping (Permission) -> Void)
  {
    check(permission: permission, authorized: authorized, limited: authorized, denied: { [weak self] _ in
      guard let self else { return }
      let title = String(
        format: stringConfig[string: .permissionIsDisabled],
        stringConfig[string: permission.localizedTitleKey])
    ...
    canceled: @escaping (Permission) -> Void)
  {
    if !permissions.isEmpty {
      ...
      check(permission: permission, stringConfig: stringConfig, authorized: { [weak self] in
        guard let self else { return }
        check(permissions: _permissions, stringConfig: stringConfig, authorized: authorized, canceled: canceled)
      }, canceled: canceled)
    } else {
      authorized()
    }
