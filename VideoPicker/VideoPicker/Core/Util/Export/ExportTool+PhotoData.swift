...
extension ExportTool {
  @discardableResult
  public static func requestPhotoData(
    for asset: PHAsset,
    ...
    -> PHImageRequestID
  {
    ...
    if #available(iOS 13.0, *) {
      return PHImageManager.default()
        .requestImageDataAndOrientation(for: asset, options: requestOptions) { data, dataUTI, orientation, info in
          handle(data: data, dataUTI: dataUTI, orientation: orientation, info: info, completion: completion)
        }
    } else {
      return PHImageManager.default()
        .requestImageData(for: asset, options: requestOptions) { data, dataUTI, uiOrientation, info in
