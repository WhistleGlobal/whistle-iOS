...
extension ExportTool {
    ...
    -> PHImageRequestID
  {
    ...
    return PHImageManager.default().requestExportSession(
      ...
      exportPreset: options.exportPreset.rawValue)
  ...
  private static func exportVideoData(
    for exportSession: AVAssetExportSession,
    options: VideoURLFetchOptions,
    completion: @escaping (Result<VideoURLFetchResponse, AnyImageError>) -> Void)
  {
    // Check Path
