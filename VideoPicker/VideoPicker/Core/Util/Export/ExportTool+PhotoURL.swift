...
extension ExportTool {
  @discardableResult
  public static func requestPhotoURL(
    for asset: PHAsset,
    ...
    -> PHImageRequestID
  {
    ...
    return ExportTool.requestPhotoData(for: asset, options: photoDataOptions) { result, requestID in
      ...
      case .success(let response):
        ...
        completion(
          .success(.init(url: outputURL, dataUTI: response.dataUTI, orientation: response.orientation)),
          requestID)
