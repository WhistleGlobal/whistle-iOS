...
final class Toast: UIView {
  ...
  private var font: UIFont = UIDevice.current.userInterfaceIdiom == .phone ? .systemFont(ofSize: 15) : .systemFont(ofSize: 18)
  ...
  override public func draw(_ rect: CGRect) {
    ...
    (message as NSString).draw(
      in: rect.insetBy(dx: textMargin, dy: textMargin),
      withAttributes: [
...
extension Toast {
  private func show(message: String, in view: UIView?, duration: TimeInterval, offset: CGPoint, completion: (() -> Void)?) {
    ...
    let tmp = message.boundingRect(
      with: CGSize(width: frame.width - textMargin * 2, height: frame.height - textMargin * 2),
