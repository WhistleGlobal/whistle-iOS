...
final class PhotoEditorContentView: UIView {
  ...
  /// 文本视图
  var textImageViews: [TextImageView] = []
  /// 文本删除视图
  private(set) lazy var textTrashView: TextTrashView = {
    let view = TextTrashView(
      options: options,
      frame: CGRect(x: (bounds.width - 160) / 2, y: bounds.height, width: 160, height: 80))
    view.alpha = 0
  ...
  let image: UIImage
  /// Context
  let context: PhotoEditorContext
  /// 配置项
  ...
  /// 裁剪数据
  var cropContext: PhotoEditorCropContext = .init()
  ...
  func updateView(with edit: PhotoEditingStack.Edit, completion: (() -> Void)? = nil) {
    updateSubviewFrame()
    ...
    group.notify(queue: .main) { [weak self] in
      ...
      if edit.cropData.didCropOrRotate, cropContext.lastCropData != edit.cropData {
        cropContext.lastCropData = edit.cropData
        layoutEndCrop(edit.isEdited)
        updateSubviewFrame()
      }
      completion?()
    }
