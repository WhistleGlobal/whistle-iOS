...
extension PhotoEditorContentView {
  ...
  func cropDone(completion: ((Bool) -> Void)? = nil) {
    isCrop = false
    didCrop = cropRect.size
      .roundTo(places: 1) != (scrollView.contentSize.reversed(!rotateState.isPortrait).roundTo(places: 1))
    setCropHidden(true, animated: false)
    layoutEndCrop()
    UIView.animate(withDuration: 0.25, animations: {
      self.updateSubviewFrame()
...
extension PhotoEditorContentView {
  /// 白色裁剪框4个角的pan手势
  @objc
  func panCropCorner(_ gr: UIPanGestureRecognizer) {
...
extension PhotoEditorContentView {
  ...
  private func layoutStartCropped() {
    ...
    // minimumZoomScale
    scrollView.maximumZoomScale = maximumZoomScale
    scrollView.minimumZoomScale = getMinimumZoomScale(
      with: lastCropData.rect.size,
      imageSize: lastCropData.imageViewFrame.size)
    ...
    let rectPathRect = CGRect(
      origin: CGPoint(
        x: (cropRealRect.minX - oldImageViewFrame.minX) / scale + cropOffsetX,
        y: (cropRealRect.minY - oldImageViewFrame.minY) / scale + cropOffsetY),
      size: CGSize(
        width: cropRealRect.width / scale,
    ...
    let newRectPathRect = CGRect(
      origin: CGPoint(
        x: lastCropData.contentOffset.x / scale + cropOffsetX,
        y: lastCropData.contentOffset.y / scale + cropOffsetY),
      size: CGSize(
        width: lastCropSize.width / scale,
  ...
  func layoutEndCrop(_ fromCache: Bool = false) {
    ...
    let contentSize = CGSize(width: bounds.width, height: bounds.width * cropRect.height / cropRect.width)
      .reversed(!isPortrait)
    let imageSize = CGSize(
      width: contentSize.width * imageFrameSize.width / reversedCropRect.width,
    ...
    UIView.animate(withDuration: fromCache ? 0 : 0.25) {
      ...
      self.cropContext.croppedSize = CGSize(
        width: self.cropRealRect.minX - self.imageView.frame.minX,
    ...
    let rectPathRect = CGRect(
      origin: CGPoint(
        x: contentOffset.x / scale + cropOffsetX,
        y: contentOffset.y / scale + cropOffsetY),
      size: CGSize(
        width: reversedCropRect.width / scale,
    ...
    let newRectPathRect = CGRect(
      origin: CGPoint(
        x: (cropRealRect.minX - imageView.frame.minX) / scale + cropOffsetX,
    ...
    newCropPath.append(newRectPath)
    if !fromCache {
      let cropAnimation = CABasicAnimation.create(
        duration: 0.25,
        fromValue: cropLayerLeave.path,
        toValue: newCropPath.cgPath)
  ...
  private func layoutRotation() {
    ...
    UIView.animate(withDuration: 0.25) {
      ...
      if case .custom(let w, let h) = self.cropOption {
  ...
  func setupCropLayer() {
    ...
    let rectPathRect = CGRect(
      origin: CGPoint(x: cropOffsetX, y: cropOffsetY),
  ...
  private func setMirrorCropRect(_ rect: CGRect, animated _: Bool = false) {
    let rectSize = rect.size.reversed(!rotateState.isPortrait)
    let rectPathRect = CGRect(
      origin: scrollView.contentOffset.multipliedBy(1 / scrollView.zoomScale),
      size: CGSize(
        width: rectSize.width / imageView.frame.width * imageView.bounds.width,
...
extension PhotoEditorContentView {
  ...
  private func getMinimumZoomScale(with cropSize: CGSize, imageSize: CGSize) -> CGFloat {
    ...
    if isHorizontal {
      ...
      return imageFitSize.height < cropSize.height
        ? (cropSize.height / imageOriginSize.height)
        : (cropSize.width / imageOriginSize.width)
    } else {
      let imageFitSize = CGSize(width: imageSize.width / imageSize.height * cropSize.height, height: cropSize.height)
      return imageFitSize.width < cropSize.width
        ? (cropSize.width / imageOriginSize.width)
        : (cropSize.height / imageOriginSize.height)
    }
  }
  ...
  private func updateCropRect(_ point: CGPoint, _ position: CropCornerPosition) {
    ...
    if rotateState.isPortrait {
      ...
      let offsetY = rotateState == .portrait
        ? contentOffset.y
        : imageFrame.height - cropStartPanRect.height - contentOffset.y
      ...
      let offsetX = rotateState == .landscapeLeft
        ? contentOffset.y
        : imageFrame.height - cropStartPanRect.width - contentOffset.y
      let offsetYUp = rotateState == .landscapeLeft
        ? contentOffset.x
        : imageFrame.width - cropStartPanRect.height - contentOffset.x
      let offsetYDown = rotateState == .landscapeLeft
        ? imageFrame.width - cropStartPanRect.height - contentOffset.x
        : contentOffset.x
  ...
  private func updateCropRectWithCropOption(_ point: CGPoint, _ position: CropCornerPosition) {
    ...
    // 第一个条件控制最小边界；第二个条件控制最大边界
    if rotateState.isPortrait {
      let offsetX = rotateState != .upsideDown
        ? contentOffset.x
        : imageFrame.width - cropStartPanRect.width - contentOffset.x
      let offsetY = rotateState != .upsideDown
        ? contentOffset.y
        : imageFrame.height - cropStartPanRect.height - contentOffset.y
  ...
  private func updateScrollViewAndCropRect(_ position: CropCornerPosition?) {
    ...
    UIView.animate(withDuration: 0.5, animations: {
      ...
      self.scrollView.minimumZoomScale = self.getMinimumZoomScale(
        with: newCropRect.size,
...
extension PhotoEditorContentView {
  ...
  private var isCrop: Bool {
    get { cropContext.isCrop }
    set { cropContext.isCrop = newValue }
  }

  /// 图片已经裁剪
  private var didCrop: Bool {
    get { cropContext.didCrop }
    set { cropContext.didCrop = newValue }
  }

  /// 图片已经裁剪或者旋转
  private var cropOrRotate: Bool {
    didCrop || rotateState != .portrait
  }

  /// 裁剪框的位置
  private var cropRect: CGRect {
    get { cropContext.cropRect }
    set { cropContext.cropRect = newValue }
  }

  /// pan手势开始时裁剪框的位置
  private var cropStartPanRect: CGRect {
    get { cropContext.cropStartPanRect }
    set { cropContext.cropStartPanRect = newValue }
  }

  /// 裁剪框与imageView真实的位置
  private var cropRealRect: CGRect {
    get { cropContext.cropRealRect }
    set { cropContext.cropRealRect = newValue }
  }

  /// 裁剪尺寸
  private var cropOption: EditorCropOption {
    get { cropContext.cropOption }
    set { cropContext.cropOption = newValue }
  }

  /// 上次裁剪开始时图片的Bounds
  private var lastScrollViewBounds: CGRect {
    get { cropContext.lastScrollViewBounds }
    set { cropContext.lastScrollViewBounds = newValue }
  }

  /// 上次裁剪开始时图片的Bounds
  private var lastImageViewBounds: CGRect {
    get { cropContext.lastImageViewBounds }
    set { cropContext.lastImageViewBounds = newValue }
  }

  /// 上次裁剪的数据，用于再次进入裁剪
  private var lastCropData: CropData {
    get { cropContext.lastCropData }
    set { cropContext.lastCropData = newValue }
  }

  /// 旋转方向
  private var rotateState: RotateState {
    get { cropContext.rotateState }
