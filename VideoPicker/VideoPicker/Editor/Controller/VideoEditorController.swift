...
protocol VideoEditorControllerDelegate: AnyObject {
  ...
  func videoEditor(
    _ editor: VideoEditorController,
    didFinishEditing video: URL,
    clipRange: ClosedRange<CGFloat>,
    isEdited: Bool)
}
...
final class VideoEditorController: AnyImageViewController {
  ...
  init(
    resource: EditorVideoResource,
    placeholderImage: UIImage?,
    options: EditorVideoOptionsInfo,
    delegate: VideoEditorControllerDelegate)
  {
      ...
      DispatchQueue.main.async {
        ...
        case .success(let url):
          ...
          self.getProgressImage(url: url) { [weak self] image in
            guard let self else { return }
            videoPreview.setThumbnail(image)
            videoPreview.setupPlayer(url: url)
            setupProgressImage(url: url, image: image)
            cropToolView.progressView.setCropProgress(options.clipRange)
          }
...
extension VideoEditorController: VideoEditorToolViewDelegate {
  func videoEditorTool(_: VideoEditorToolView, optionDidChange option: EditorVideoToolOption?) {
...
extension VideoEditorController: VideoEditorCropToolViewDelegate {
  func cropTool(_: VideoEditorCropToolView, playButtonTapped button: UIButton) {
  ...
  func cropToolDurationOfVideo(_: VideoEditorCropToolView) -> CGFloat {
    CGFloat(videoPreview.player?.currentItem?.duration.seconds ?? 0)
  }
}
...
extension VideoEditorController {
  ...
  private func setupProgressImage(url: URL, image: UIImage) {
    ...
    getVideoThumbnailImage(url: url, count: count) { idx, image in
      ...
      let resizedImage = UIImage.resize(
        from: image,
        limitSize: CGSize(width: itemSize.width * scale, height: itemSize.height * scale),
        isExact: true)
  ...
  private func getVideoThumbnailImage(url: URL, count: Int, completion: @escaping (Int, UIImage) -> Void) {
    ...
    asset.loadValuesAsynchronously(forKeys: ["duration"]) {
      ...
      let array = (1 ... count).map { NSValue(time: CMTime(
        seconds: Double($0) * (seconds / Double(count)),
  ...
  private func addPlayerObserver() {
    if videoPreview.player != nil, !didAddPlayerObserver {
      ...
      videoPreview.player?.addPeriodicTimeObserver(
        forInterval: CMTime(value: 1, timescale: 60),
        queue: nil,
        using: { [weak self] _ in
          guard let self else { return }
          guard videoPreview.isPlaying else { return }
          guard let current = videoPreview.player?.currentItem?.currentTime() else { return }
          guard let total = videoPreview.player?.currentItem?.duration else { return }
          let progress = CGFloat(current.seconds / total.seconds)
          let progressView = cropToolView.progressView
          cropToolView.progressView.setProgress(progress)
          if progress >= progressView.rightProgress {
            videoPreview.player?.pause()
            cropToolView.playButton.isSelected = videoPreview.isPlaying
            cropToolView.progressView.setProgress(cropToolView.progressView.leftProgress)
            videoPreview.setProgress(cropToolView.progressView.leftProgress)
          }
        })
    }
  }
  ...
  /// 增加视频轨道
  private func addVideoComposition(
    ...
    completion: @escaping (Result<URL, Error>) -> Void)
  {
    ...
    guard let exportSession = AVAssetExportSession(asset: composition, presetName: AVAssetExportPresetPassthrough)
    else { return }
