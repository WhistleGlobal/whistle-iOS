...
open class ImageEditorController: AnyImageNavigationController {
  ...
  public convenience init(
    photo resource: EditorPhotoResource,
    options: EditorPhotoOptionsInfo,
    delegate: ImageEditorControllerDelegate)
  {
    self.init()
    update(photo: resource, options: options)
    editorDelegate = delegate
  }

  /// Init Video Editor
  public convenience init(
    video resource: EditorVideoResource,
    placeholderImage: UIImage?,
    options: EditorVideoOptionsInfo,
    delegate: ImageEditorControllerDelegate)
  {
    self.init()
    update(video: resource, placeholderImage: placeholderImage, options: options)
    editorDelegate = delegate
  }
...
extension ImageEditorController {
  ...
  public func update(video resource: EditorVideoResource, placeholderImage: UIImage?, options: EditorVideoOptionsInfo) {
    enableDebugLog = options.enableDebugLog
    let checkedOptions = check(resource: resource, options: options)
    let rootViewController = VideoEditorController(
      resource: resource,
  ...
  private func check(resource: EditorPhotoResource, options: EditorPhotoOptionsInfo) -> EditorPhotoOptionsInfo {
    ...
    if options.rotationDirection != .turnOff {
      for cropOption in options.cropOptions {
        if case .custom(let w, let h) = cropOption, !options.cropOptions.contains(.custom(w: h, h: w)) {
          fatalError(
            "Custom crop option must appear in pairs if you turn on the rotation feature. Please add .custom(\(h), \(w)) to cropOptions")
        }
      }
    }
    #else
    ...
    if options.cropOptions.count == 1 {
      if case .custom(let w, let h) = options.cropOptions.first, w != h {
...
extension ImageEditorController: PhotoEditorControllerDelegate {
  func photoEditorDidCancel(_: PhotoEditorController) {
    editorDelegate?.imageEditorDidCancel(self)
  }

  func photoEditor(_: PhotoEditorController, didFinishEditing photo: UIImage, isEdited: Bool) {
...
extension ImageEditorController: VideoEditorControllerDelegate {
  func videoEditorDidCancel(_: VideoEditorController) {
    editorDelegate?.imageEditorDidCancel(self)
  }

  func videoEditor(_: VideoEditorController, didFinishEditing video: URL, clipRange: ClosedRange<CGFloat>, isEdited: Bool) {
