...
// MARK: - PHAsset + EditorPhotoResource
    ...
    ExportTool.requestPhoto(for: self, options: .init(size: limitSize)) { [unowned self] result, _ in
      ...
      case .success(let response):
        if !response.isDegraded {
          completion(.success(adjustImage(response.image)))
        }
      case .failure(let error):
        completion(.failure(error))
        if error == .cannotFindInLocal {
          loadImageFromNetwork(completion: completion)
        }
      }
    }
  }
...
extension EditorPhotoResource {
  ...
  fileprivate func adjustImage(_ sourceImage: UIImage) -> UIImage {
    fixOrientationAndRotateToUp(sourceImage)
  }
  ...
  private func fixOrientationAndRotateToUp(_ sourceImage: UIImage) -> UIImage {
    ...
    if
      let imageData = sourceImage.jpegData(compressionQuality: 0.1),
      ...
      let orientationValue = exifProperties[kCGImagePropertyTIFFOrientation as String] as? UInt32
    {
      ...
      let newSize = CGRect(origin: .zero, size: sourceImage.size).applying(CGAffineTransform(rotationAngle: rotationAngle))
        .size
    ...
    let fixedImage = UIGraphicsImageRenderer(size: canvasSize, format: getImageRendererFormat()).image { rendererContext in
      ...
      context.draw(
        imageRef,
        in: CGRect(x: -imageSize.width / 2, y: -imageSize.height / 2, width: imageSize.width, height: imageSize.height))
