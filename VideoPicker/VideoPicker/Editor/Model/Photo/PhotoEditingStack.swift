...
extension PhotoEditingStack.Edit {
  var isEdited: Bool {
    cropData.didCrop || cropData.rotateState != .portrait || !brushData.isEmpty || !mosaicData.isEmpty || !textData.isEmpty
  }
...
extension PhotoEditingStack {
  ...
  private func rotateImage(_ image: UIImage) -> UIImage {
    ...
    return UIGraphicsImageRenderer(size: newSize, format: getImageRendererFormat()).image { rendererContext in
